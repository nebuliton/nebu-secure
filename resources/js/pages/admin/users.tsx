...
const defaultForm: UserPayload = { name: '', email: '', password: '', role: 'user', is_active: true };
...
export default function AdminUsersPage() {
    ...
    const saveUserMutation = useMutation({
        mutationFn: async (payload: UserPayload) => {
            if (editingUser) {
                return apiRequest<VaultUser>(`/api/admin/users/${editingUser.id}`, 'PUT', payload);
            }
    ...
    const toggleUserMutation = useMutation({
        mutationFn: async (user: VaultUser) => apiRequest(`/api/admin/users/${user.id}/toggle-active`, 'PATCH'),
        onSuccess: () => {
    ...
    const issueTemporaryPasswordMutation = useMutation({
        mutationFn: async (user: VaultUser) =>
            apiRequest<{ password: string }>(`/api/admin/users/${user.id}/issue-temporary-password`, 'POST'),
        onSuccess: (data) => {
            window.prompt('Temporäres Passwort (jetzt kopieren):', data.password);
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="space-y-6 p-6">
                <Card className="overflow-hidden border-border/70 bg-gradient-to-r from-sky-100 to-cyan-100 dark:from-slate-900 dark:to-slate-800">
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2 text-2xl"><UserCog className="size-6" />Benutzerverwaltung</CardTitle>
                        <p className="text-sm text-muted-foreground">Konten anlegen, Rollen steuern, Zugriff sperren und Benutzer entfernen.</p>
                    </CardHeader>
                </Card>
                    ...
                    <CardHeader className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                        ...
                        <Dialog open={open} onOpenChange={setOpen}>
                            <DialogTrigger asChild>
                                <Button onClick={onCreateClick}><ShieldPlus className="mr-2 size-4" />Benutzer erstellen</Button>
                            </DialogTrigger>
                            <DialogContent>
                                <DialogHeader>
                                ...
                                >
                                    ...
                                    <Button className="w-full" type="submit" disabled={saveUserMutation.isPending}>{saveUserMutation.isPending ? 'Speichern...' : 'Speichern'}</Button>
                                </form>
                            </DialogContent>
                        </Dialog>
                    </CardHeader>
                                    ...
                                    {filteredUsers.map((user) => (
                                        <tr key={user.id} className="border-t border-border/50 align-top">
                                            ...
                                            <td className="flex flex-wrap gap-2 px-4 py-3">
                                                ...
                                                <Button variant="outline" size="sm" onClick={() => toggleUserMutation.mutate(user)}>{user.is_active ? 'Deaktivieren' : 'Aktivieren'}</Button>
                                                <Button
                                                    variant="outline"
                                                    size="sm"
                                                    onClick={() => {
                                                        const password = window.prompt('Neues Passwort eingeben (mind. 12 Zeichen):');

                                                        if (!password) {
                                                            return;
                                                        }

                                                        resetPasswordMutation.mutate({ user, password });
                                                    }}
                                                >
                                                ...
                                                <Button
                                                    variant="outline"
                                                    size="sm"
                                                    onClick={() => issueTemporaryPasswordMutation.mutate(user)}
                                                >
